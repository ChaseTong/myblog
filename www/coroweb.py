#reference:http://blog.csdn.net/qq_38801354/article/details/73008111
...
def add_routes(app, module_nume):
    n = module_nume.rfind('.')
    if n == (-1):
        mod = __import__(module_nume, globals(), locals())
    else:
        name = module_nume[n+1:]
        mod = getattr(__import__(module_nume[:n], globals(), locals(), [name]), name)
    for attr in dir(mod):
